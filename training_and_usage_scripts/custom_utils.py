...
def get_CAM_map_with_prediction_convnext(model, images, mod_t_for_layers=[3], target_layer=3, is_recurrent=True, 
                                         t=3, change_t_blocks=[0,1,2,3,4], normalize=False, kval=1, return_pred_prob=False):
    if(isinstance(model, torch.nn.parallel.DistributedDataParallel)):
        #print("Distributed parallel model")
        model = model.module
    final_layer = model._modules.get('stages')[3].blocks
    if(is_recurrent):
        #model.time_steps = t
        for mod_layer in mod_t_for_layers:
            #print("Recurrent t for each block of layer {}".format(mod_layer))
            layer = model._modules.get('stages')[mod_layer].blocks
    ...
    weight_softmax_params = list(model._modules.get('head').fc.parameters())
...
def get_CAM_map_with_prediction_efficientnet(model, images, mod_t_for_layers=[6], target_layer='conv_head', is_recurrent=True, t=2, change_t_blocks=[0,1,2,3,4], normalize=False, kval=1, return_pred_prob=False):
    if(isinstance(model, torch.nn.parallel.DistributedDataParallel)):
        model = model.module
    ...
    if(is_recurrent):
        ...
        for mod_layer in mod_t_for_layers:
            #print("Recurrent t for each block of layer {}".format(mod_layer))
            layer = model.blocks[mod_layer]#model.module._modules.get(mod_layer)
            for i, mod_bl in enumerate(layer): 
                if(i in change_t_blocks):
                    mod_bl.time_steps = t
                else:
...
def get_CAM_map_with_prediction_resnet(model, images, mod_t_for_layers=['layer4'], target_layer='layer4', is_recurrent=True, t=3, change_t_blocks=[0,1,2,3,4], normalize=False, kval=1, return_pred_prob=False):
    if(isinstance(model, torch.nn.parallel.DistributedDataParallel)):
        #print("Distributed parallel model")
        model = model.module
    final_layer = model._modules.get(target_layer)
...
def filter_pred_with_iou(model, images, time_steps, image_res=224, model_type = 'resnet', iou_thresh=0.5):
    """
    Filters outputs from model to only retain unique bounding boxes (based on IOU threshold)
    """
    time_step_img_dict = {} #keyed by outs (list of time step outputs) and bboxes 
    
    #input(model_type)
    if(model_type in ['resnet']):
        get_CAM_map_with_preds_func = get_CAM_map_with_prediction_resnet
    elif(model_type in ['efficientnet']):
        get_CAM_map_with_preds_func = get_CAM_map_with_prediction_efficientnet
    elif(model_type in ['convnext']):
        get_CAM_map_with_preds_func = get_CAM_map_with_prediction_convnext
        
        
    for current_time_step in range(1, time_steps+1):
        or_model_l4, pred_or, pred_prob, prediction_all = get_CAM_map_with_preds_func(model, images, is_recurrent=True, 
        ...
        for i_img, or_img in enumerate(images):
            ...
            if(i_img not in time_step_img_dict):
                ...
                for j, existing_bbox_out in enumerate(time_step_img_dict[i_img]['bbox']):
                    if(get_iou(existing_bbox_out, new_bbox_coords)>iou_thresh):
...
def accuracy_over_iou_filtered(outputs, target, topk=(1,5)):
    """Computes the accuracy over the k top predictions for the specified values of k"""
    with torch.no_grad():
        maxk = max(topk)
        batch_size = target.size(0)
